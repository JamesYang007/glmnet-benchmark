...
timer = function(X, y) {
    time.out = microbenchmark(glmnet(X, y, family='gaussian', tol=1e-15, standardize=F), times=1L, unit='s')
    glmnet.out = glmnet(X, y, family='gaussian', tol=1e-15, standardize=F)
